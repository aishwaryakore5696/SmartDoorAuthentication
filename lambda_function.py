...
def detect_new_image(local_file_path, path_output_dir,fragment_name):
   try:
        ...
        kvs_archive_client = boto3.client('kinesis-video-archived-media',
                                          'us-east-1',endpoint_url='https://b-604520a7.kinesisvideo.us-east-1.amazonaws.com')
...
def insert_entry_in_dynamo_db1(table_name, insert_json):
    try:
        ...
        table = dynamodb.Table(table_name)
        print("TYPE" ,type(insert_json['phno']))
        print(int(insert_json['phno']))
        print(type(int(insert_json['phno'])))
        response_initial = client.get_item(TableName='DB1', Key={'phno': {'N': insert_json['phno']}})
        print("Get Item Response " , response_initial)
        if "Item" not in response_initial.keys():
                response = table.put_item(
                                Item={
                                    'otp': int(insert_json['otp']),
                                    'phno': int(insert_json['phno']),
                                    'expirytimestamp': insert_json['expirytimestamp']
                                    # "status": insert_json['status']
                                }
                                    )
        else:
            print("The user has receiced an OTP So SNS would not be sent to the visitor")
            return False
        if response['ResponseMetadata']['HTTPStatusCode'] == 200:
            print("Insert in Dynamo DB table "+ table_name +" was successful")
            return True
    except Exception as e:
        print(" The execution of the function insert entry into dynamo db failed due to error", str(e))
        return True
    ...
    try:
        ...
        if len(decoded_data['FaceSearchResponse']) !=0 and len(decoded_data[
            ...
            if len(add_image_result['FaceMatches']) == 0:
                ...
                if insert_result:
                    ...
                    message = "A new person has been detected at your entrance.\n To " \
                              ...
                              "URL: " + OWNER_WEBPAGE_URL + "?faceid=" + face_id + "\n" \
                              "Photo Link: " + s3_path
            ...
            if not visitor_details_response:
                ...
                if visitor_phno.startswith("+1"):
                    visitor_phno = visitor_phno[2:]
                print("Visitor phone no " , visitor_phno)
                print(type(visitor_phno))
                ...
                if insert_otp_result:
                    ...
                    message = "Hi " + vistor_name + "\n" \
                              "Please enter the otp" + str(otp) + " by clicking on the URL below\n" \
                               "URL: " + VISITOR_WEBPAGE_URL + "?faceid=" + visitor_face_id

                    print("Phone number is: +1"+str(visitor_phno))
                    print("URL created: ", VISITOR_WEBPAGE_URL+ "?faceid=" + visitor_face_id)
                    sns_result = send_sns_message(message,phone_number="+1"+str(visitor_phno))
                    if sns_result:
                        print(
